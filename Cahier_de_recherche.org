...
#+end_center
As we can see on the graph we can now observe differents results for all the scheduling methods for the lublin dataset but the results provide are really high compare to the other scheduling methods:
#+begin_example
LIN          5893.194787
