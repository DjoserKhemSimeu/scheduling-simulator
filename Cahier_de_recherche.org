...
I think the result provide by the pearson test is only for MAE with little difference (+ ou - 0.02), obviously with MAE of 0.2 and MAE of 0.8 we will observe some difference. It can be due to the way which we choosed to construct the trainning dataset, which is not totally realistic :
+ just take into account the position of the job in the best permutation of Q to compute the priority
** What to do next ?
here we have two choice :
+ Make more realistic the construction of the dataset to increase the correlation between MAE and AVGBoundedSlowdown
+ Try with the serial version of the trainning dataset to make model that ensure the seriality of the jobs
** Comparison of our models to SAF and F2
#+begin_example
LIN          1164.351312
S3_V2_D1      257.833541
S3_V10_D4     542.374548
...
F2            165.814373
#+end_example
S3_V2_D1 is close to beat them
** New 3D représentation
+ MAE
+ VIF
+ AVGBoundedSlowdown
+ AVGD
Modifications of the tester.py code
#+begin_example
if __name__ == "__main__":
...
** Results
#+begin_center
[[file:images/4D_MSE.png]]
#+end_center
** TODO :: Implement MEM4 which take in the memory system the info about the current task an compare it with MEM2
** TODO :: Do the 4D plot for each experimental context
** TODO :: Use the backfilling parameter in the tester now
* mercredi 22 mai
** Comparison of MEM4 and MEM2
#+begin_center
[[file:images/MEM2_VS_MEM4+Back.png]]
#+end_center
Mean:
#+begin_example
MEM2    121.377152
MEM4    211.353406
#+end_example
As we can see MEM2 have a lowest AVGBoundedSlowdown than MEM4, it can be due to the fact that MEM4 smooth the effect of the "memory mechanism" by taking in account the information about the current task.
** Work on the memory regressions
+ x = p_mean
+ w = q_mean
+ z = r_mean
| VIF\AVGD | 1                                          | 2                               | 3                                   | 4 |
|        1 | ('sqrt(p)', 'sqrt(w)', 'x2', 'z2q') (1,5)  | ('q1', 'w1', 'p2x', 'z2r') (2)  | ('x2p', 'q2r', 'w2q', 'r2z') (3)    | X |
|        2 | ('sqrt(z)', 'p1q', 'p1r', 'q1x') (1,..)    | ('r1', 'x2', 'z2', 'x2q') (2)   | ('r2p', 'q2x', 'q2w', 'r2z') (3)    | X |
|        3 | ('sqrt(x)', 'p1q', 'r1w', 'p2q') (1.75)    | ('r1', 'q1z', 'r1x', 'z2q') (2) | ('p2r', 'w2p', 'z2r', 'z2x') (3)    | X |
|        4 | ('sqrt(w)', 'sqrt(z)', 'p1', 'w2x') (1.25) | ('w1', 'p1x', 'p1w', 'p2w') (2) | ('w2p', 'r2x', 'w2r', 'z2x') (3)    | X |
|        5 | ('sqrt(r)', 'sqrt(x)', 'x1w', 'r2z') (1.5) | ('r1', 'z2', 'r1w', 'p2q') (2)  | ('p2w', 'q2z', 'z2r', 'z2w') (3)    | X |
|        6 | ('sqrt(q)', 'sqrt(r)', 'z1', 'p1z') (1)    | ('r2', 'p1z', 'r1z', 'x1z') (2) | ('w2p', 'q2r', 'x2q', 'q2z') (3)    | X |
|        7 | ('sqrt(x)', 'sqrt(w)', 'q1', 'x1z') (1)    | ('x1', 'q1r', 'q1w', 'q2r') (2) | ('p2r', 'q2r', 'q2w', 'w2q') (3)    | X |
|        8 | ('sqrt(q)', 'sqrt(z)', 'z1', 'r1w') (1)    | ('r1', 'p1q', 'r1w', 'w2z') (2) | ('q1x', 'q2p', 'q2r', 'q2x') (2.75) | X |
|        9 | ('sqrt(p)', 'sqrt(w)', 'w1', 'q2') (1)     | ('q1', 'q1x', 'w1z', 'x2q')(2)  | ('p1q', 'q2p', 'x2p', 'p2w') (2.75) | X |
|       10 | ('sqrt(r)', 'sqrt(w)', 'x1', 'z1') (0.75)  | ('r1', 'z1', 'p2', 'r1z') (2)   | ('p2r', 'w2p', 'p2z', 'w2x') (3)    | X |

** TODO :: Select the 10% best parameters and analyse them
** TODO :: Understand the error of MEM4 SAF
** TODO :: Run the tester on all the serial methodsa
* jeudi 23 mai
** Training of the serial regression
#+begin_example
if __name__ == "__main__":
    workload_experiments(
        ["CTC-SP2", "SDSC-BLUE", "LUBLIN 256"],
...
#+begin_center
 [[file:images/Serial+Back.png]]
#+end_center
as we can see, some of the regression parameters found provide a lower AVGBoundedSlowdown than MEM2 it's really interesting.
** Observing the composition of the 25% best regresion parameters
*** Classic Regression
#+begin_example
S3_V3_D3     164.048508
S3_V2_D1     257.833541
...
Which correspond to :
#+begin_example
1 : [‘sqrt(p)’, ‘sqrt(w)’, ‘w1’, ‘q2’]
2 : [‘w2p’, ‘q2r’, ‘x2q’, ‘q2z’]
3 : [‘p2w’, ‘q2z’, ‘z2r’, ‘z2w’]
4 : [‘sqrt(p)’, ‘sqrt(w)’, ‘x2’, ‘z2q’]
5 : [‘q1x’, ‘q2p’, ‘q2r’, ‘q2x’]
6 : [‘sqrt(r)’, ‘sqrt(x)’, ‘x1w’, ‘r2z’]
7 : [‘r1’, ‘p1q’, ‘r1w’, ‘w2z’]
#+end_example
 occurence :
 #+begin_example
...
w2z: 1
 #+end_example
** For next time
*** TODO :: Analysis of the coefficients of the two type of regression
*** TODO :: Better explenation of the proof
*** TODO :: See with Danilo the structure of my report
*** TODO :: Start writting the report
* vendredi 24 mai
...
**** Scheduling methods
**** Genetic algorithm
*** Dataset generation and Schduling policies
...
***** Finding the best models inputs with a machine learning analysis
****** VIF/AVGD table
******* VIF
******* AVGD
****** A serial point of view of the jobs
******* "Memory" mecanism
******* Modifications of the training datase
*** Analysis of regression results
**** Results
**** Parameters analysis
**** Starvation analysis
**** MAE analysis
*** Cunclusion and discussion
* lundi 27 mai
** New Structure
*** Abstract
*** Intro
*** Preliminary concepts
*** Methods
**** Using ML to understand the heuristics
+ Dataset generation + Classical linear regression
**** Adding the dimension of serialism
+ MEM 2 + SER regression
*** Conclusion and discussion
** TODO :: Next time
+ Graph about the inputs of the 10% lowest AVGBSLD for all the experimental context for S3 and SER
+ Re construct the repport
* Dimanche 2 juin
** Work on the normalisation of the dataset
the goal is to put the job attributesin the same scale increase the coherence of the coefficient analysis
*** Training part
+ Modification of the training dataset:
  #+begin_example

  #+end_example
+ I observed really strange coefficient running the training on full normalized data
  #+begin_example
  #+end_example
* Mercredi 12 juin
** Work on the normalization
Implementation in the c scheduling code of the normalization method.
#+begin_example
...
#+end_example
** Debugging
I observed really strange results and i see by using the verbosity argument that:
#+begin_example
[node-0:master:(1) 0.000000] [msg_test/INFO] P :
[node-0:master:(1) 0.000000] [msg_test/INFO] tab = 11975.000000
[node-0:master:(1) 0.000000] [msg_test/INFO] Mean = 4584.989086
[node-0:master:(1) 0.000000] [msg_test/INFO] STD = 7768.798152
[node-0:master:(1) 0.000000] [msg_test/INFO] Q :
[node-0:master:(1) 0.000000] [msg_test/INFO] tab = 0.000000
[node-0:master:(1) 0.000000] [msg_test/INFO] Mean = 0.000000
[node-0:master:(1) 0.000000] [msg_test/INFO] STD = 0.000000
[node-0:master:(1) 0.000000] [msg_test/INFO] R :
[node-0:master:(1) 0.000000] [msg_test/INFO] tab = 0.000000
[node-0:master:(1) 0.000000] [msg_test/INFO] Mean = 0.000000
[node-0:master:(1) 0.000000] [msg_test/INFO] STD = 0.000000
...
#+end_example
*** Solution
the q and r values are int so we must define a function to transform them:
#+begin_example
...
** observation
#+begin_center
[[file:images/results_norm.png]]
#+end_center
As we can see on the graph we can now observe differents results for all the scheduling methods for the lublin dataset but the AVGBSL results provide are really high compare to the other scheduling methods:
#+begin_example
LIN          5893.194787
S3_V1_D3     3685.973657
S3_V1_D4     3057.119365
S3_V2_D1     3233.989603
S3_V2_D2     3801.127168
S3_V2_D3     3198.229417
S3_V2_D4     3443.098364
S3_V3_D1     3407.309286
S3_V3_D2     3084.233719
S3_V3_D3     3204.898610
S3_V3_D4     3215.370874
S3_V4_D1     2534.065462
S3_V4_D2     2395.700678
S3_V4_D3     4123.725868
S3_V4_D4     3652.948055
S3_V5_D1     3252.368926
S3_V5_D2     3183.210339
S3_V5_D3     3821.676253
S3_V5_D4     3884.712590
S3_V6_D1     3185.301810
S3_V6_D2     2555.112653
S3_V6_D3     3287.161620
S3_V7_D1     2561.623333
S3_V7_D2     3155.655555
S3_V7_D3     2589.753530
S3_V7_D4     3359.185889
S3_V8_D1     3576.356939
S3_V8_D2     2616.409522
S3_V8_D3     2845.925984
S3_V8_D4     3138.401424
S3_V9_D1     3296.902480
S3_V9_D2     2959.389152
S3_V9_D3     3046.445468
S3_V9_D4     3059.937601
S3_V10_D1    3242.462925
S3_V10_D2    3237.958102
S3_V10_D3    3047.941632
S3_V10_D4    3033.934288
#+end_example
** Problem highlighted by Danilo
I normalized my trainning data on the all dataset but in the tester implementation i normalized it on a defined sample of the dataset
*** First solution use the average number of tasks on the tester to normalized my dataset
#+begin_example
import pandas as pd
import numpy as np
import math
...
* Lundi 17 juin
** coefficicient analysis
I will wait to see danilo to talk about the standardisation while I will start the coefficicent analysis with unstandardize data.
** New approach
Select the models with the lowest average bounded slowdowns, and obsever their coeficients arfter the trainning with normalized data,
#+begin_center
[[file:images/Reg_coefs_norm_grid.png]]
#+end_center
As we can see, we don't have lot of signs variations execpt for : q3 and r2q
*** TODO :: For next time
+ Find a way to remove the changelments of signs triggered by the normalization.
+ Do the same for the contex-aware models
+ anlysis of the results
* Mardi 18 juin
** Coefficient analysis
By suprressing the models S3_V3_D3 and S3_V7_D2 i succeed to reproduce the same signs as the unnormalized coefficients.
After anlysing the coefficient after normalization i made 5 major observation:
+ The models applies penalization (+) an prioritization (-)
+ The strongest prioritisaztion applyed by the models is on the area of the tasksb (pq polynomials)
+ The strongest penalisations applyied by the models are on the number of cores needed and the arrival time of the tasks
+ The models consider majoritarly the 3 attributes
+ When r is considered , we always apply a positive coefficicent on it.
** Changement of approach
now i use the minmax standardization
** Coefficient analysis for the contex-aware models
i performed the minmax normalization on the MEM dataset and i removed the policies affected by the normamlization ("SER_1_2","SER_1_3","SER_5_2","SER_5_3")
#+begin_center
[[file:images/Reg_coefs_serial_norm_grid.png]]
#+end_center
*** Analysis
+ As for the other models, the models applied penalization (+) and prioritization (-)
+ The strongest penalization applied by the models is on the  tasks scheduled in a scheduling context of task with high computational attributes
+ Majoritarly, the strongest penalization applied by the models is on the task with high computational attributes.

* Vendredi 21 juin
** Solving the normalization problem
Before, i didin't take in account the results of the priority function, i change that by :
#+begin_example
...
#+end_example
* Lundi 24 juin
** Analyzing the results of the normalization
Now, I must understand the behavior behind our models by comparing their coefficients
*** TODO :: Use the best normùalized models or the unnormalized ones?
We observe we have lot of model which uses jobs attributes as : 10*p*q -10 * p²*q , by using a 3D representation of this we can see that the results of this operation is exponantial in function of q, in our anlysis we can interpret that as a strong penalization on the number of cores needed by the jobs.
We can observe approximatively the same phenomenom in a performant context aware regression model with : 10*q*x-10*q²*x.
* Jeudi 27 juin
** TODO :: Missions
+ 3D colormap best LUBLIN models
  #+begin_center
  [[file:images/colormap_3D.png]]
  #+end_center
  This results shown that our best model on the LUBLIN dataset produces differents be'havior
+ comparison of our models between ESTIMATED and backfilling condition
  #+begin_center
  [[file:images/pecent_Back_Est.png]]
  #+end_center
  weak policies are mostly of degree 4
+ put the exact coefn instead of approximation
* Vendredi 28 juin
** Observation of our best models behavior
#+begin_center
[[file:images/colormap_3D_8funcs.png]]
#+end_center
We can see 4 type of models among the 8 best models:
+ V7_D3, V3_D3 : penalization of the tasks with a high cost for the platforms
+ V4_D3, V5_D1 : penalize the tasks proportionally to the number cores needed
+ V3_D4, V2_D2 : prioritize the jobs with high p and q but penalize the task with a low p and a high q
+ V9_D3, V2_D1 : penalization of the task with a high cost for the platform but p have more weight than q on the penalization
* Mardi 2 juillet
** Trying to deeply analyse S3_V3_D3
*** TODO :: working on the normalisation of the coeffs of S3_V3_D3
#+begin_center
[[file:images/Reg_coefs_norm_family.png]]
#+end_center
*** TODO :: plotting the colormap 2D of the major coef of S3_V3_D3
#+begin_center
[[file:images/colorbar_rxn_q3p.png]]
#+end_center
*** TODO :: plot the graph of the the variation of p with (q=1, r=0) and q (p=1 and r=0) for the results of S3_V3_D3
#+begin_center
[[file:images/S3_V3_D3_comp_impact.png]]
#+end_center
* Mercredi 3 juillet
** Objectif
*** DONE :: perform the same analysis for the best context aware model
#+begin_center
[[file:images/Reg_coefs_ser_norm_family.png]]
[[file:images/SER_9_1_comp_impact.png]]
#+end_center
** Observations
*** S3_V3_D3 model :
+ max coeff applied positively on the input q³p
+ q³p penalize the tasks with high values for their attributes p and q (left right corner of the color map)
+ r have no significant impact on the priority value
+ when p and q increase the result increase also
We can see that the S3_V3_D3 model compute the priority of a task mostlty from the value of the p and q attributes it' quite simalar to thebehavior of the SAF model with unnormalized data.
+ categories de job (processing time = seul facteur dans cette etude) plus de variations dans lme carre superieur droit

*** SER_9_1 model :
+ the function use only the attributes p, q and w
+ no significant major coeff but the highest one is applied negatively on w
+ -w prioritize the jobs that are scheduled in a scheduling context with high q values
+ when p and q increase, the results increase
+ when w increase the results decrease
We can see that the SER_9_1 compute the priority of a jobs by applying a penalization proportional to the values of the p and q attrbutes and a prioritization proportional to the values of the w attribute.
*** DONE :: ajoute la politique q³p ajout aussi q²p
*** DONE :: ajouter la heatmap de MEM2
*** DONE :: tester plusieur estimated value pour le tester
* Jeudi 4 juillet
** Starting a new approach on the estimated experimental condition
*** Hypothesis
Thanks to the heatmap, we observe that there are job categories (q>0.5) where it is more important to have an accurate prediction of the precessing time. For example for the job category q>0.5 , having inaccurate predictions of p will have greater consquences in the priority and the schedule. We made this hypothesis by observing that some model have really good results in the actual conditions but really bad results in the two other conditions (S3_V1_D3).
*** Next steps :
+ Vary the processing time estimation in different surface region (heatmap) to see the impact in the schedule (validate the hypothesis)
+ Propose a prediction method that that take the hypothesis into account
+ Compare the proposed pred method with other baseline pred methods
*** Method :
#+begin_example
if cores_norm[i]<0.5 & runtimes_norm[i]<0.5 :
        req_norm[i] = runtimes_norm[i] +np.random(0.5,1)*runtimes_norm[i]
else :
        req_norm[i] = runtimes_norm[i]
#+end_example
for the category q<0.5 and p<0.5
* Lundi 8 juillet
** Behavior of the estimated values
We want to see the effect on the AVGBSL of the accuracy of the estimated p values for different type of jobs with the Q3P policy. we defined 4 different type of jobs:
+ Zone 1 : p<0.5 and q>0.5
+ Zone 2 : p>0.5 and q>0.5
+ Zone 3 : p<0.5 and q<0.5
+ Zone 4 : p>0.5 and q<0.5
*** Implementation
to observe the phonemenom in each zone, we decided for each zone to perform a test where the estimated p values of the jobs in the current are computed like that :
#+begin_example
p_est = p + (1+x)
Where :  x a random in [0 , fact]
fact = 10
#+end_example
the function :
#+begin_example
...
*** Results :
#+begin_center
[[file:images/results_zones_q3p.png]]
#+end_center
We can see that the Zone 3 provides us the highest AVGBSL observe with this approach, it can be explain by the fact that the jobs with low p values have a big impact on the computation of the AVGBSL, reduccing the precision of the estimated p value of these jobs make their scheduling less accurate, btw increase the waiting time of these jobs on the platform.

* Mercredi 10 juillet
** Missions
*** TODO :: Observe the behavior of the heatmap of the 4 best policy on p and q
#+begin_center
[[file:images/results_best_models.png]]
#+end_center
#+begin_center
[[file:images/colorbar_rxn.png]]
#+end_center
** Observations
|                               | Q3P                        | Q2P                        | SAF             | F2                  |
| AVGBSL                        | 103                        | 106                        | 245             | 323                 |
| Normalized                    | Yes                        | Yes                        | No              | No                  |
| max q                         | Normal                     | Normal                     | Normal          | Strong penalization |
| max p                         | penalization threshold 0.7 | penalization threshold 0.5 | Normal          | Normal              |
| penalization demarcation      | strong (line)              | strong (line)              | soft (gradient) | soft (gradient)     |
| penalization zone             | right up corner            | right up corner            | right up corner | right up corner     |
| area of the penalization zone | low                        | low                        | high            | high                |

*** TODO :: find an other way to analyze the best method other as the heatmap
** dominik.huber@inria.fr
