...
#+begin_example
MEM1:priority = r + (p-p_mean) + (q- q_mean)
MEM2:priority = r + ((p*q) - (p_mean * q_mean)) (area)
#+end_example
...
As we can see the scheduling policy based on the memory mecanism M provide a lower average bounded slowdown than the FCFS and WFP3  policies, MEM2 provide the lowest AVGBoundedSlowdown ist really interesting! I must find a way to infer on the utilty of the task serialism.
